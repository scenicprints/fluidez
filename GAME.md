...
**Status: ALL 122 MISSIONS WRITTEN.** Design agreed, the whole city mapped,
the mockup playable with every mission and the whole street crowd baked in.
Nothing is built into the app yet — that is what is left.
...
**2. The app screen is now the only thing standing between this and playable.**
The canvas world, the chunk tray, and swapping Scenes out of `TAB_DEFS`.
There is more than enough content to justify it.
