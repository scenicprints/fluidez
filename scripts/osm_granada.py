...
def stamp_roads(grid, edges):
    ...
    for a, b, cls, pts in edges:
        ...
        for i in range(1, len(pts)):
            x0, y0 = pts[i - 1]
            x1, y1 = pts[i]
            # Sample the segment rather than drawing it. A drawn line takes one
            # tile at each diagonal step; the traffic interpolates in floating
            # point and passes through BOTH elbow tiles, so a caponera clipped
            # the corner of a house about half a percent of the time. Walking
            # the same interpolation the traffic uses cannot miss a tile it
            # will drive over.
...
APP_MAP = os.path.join(ROOT, 'docs', 'js', 'game', 'map.js')

APP_HEADER = """// Granada, as surveyed. GENERATED by scripts/osm_granada.py -- do not edit.
//
// This is the same packed map the mockup carries: run-length encoded terrain,
// the facade colours, the street graph the traffic drives on, and the named
// places an NPC can stand at. See the generator for the format.
//
// It is a module of its own and imported dynamically, so the half megabyte
// only lands on a phone that actually opens the game -- and never on a phone
// learning Schweizer Hochdeutsch.
//
...
def write_app(blob):
    """The same blob the mockup gets, as an ES module for the real app."""
    d = os.path.dirname(APP_MAP)
    if not os.path.isdir(d):
        os.makedirs(d)
    with io.open(APP_MAP, 'w', encoding='utf-8', newline=NEWLINE) as f:
        f.write(APP_HEADER + blob + '`;' + NEWLINE)
    return APP_MAP
...
def main():
    ...
    ap.add_argument('--app', action='store_true',
                    help='also write docs/js/game/map.js for the real app')
    a = ap.parse_args()
    ...
    if a.app:
        L.append(u'wrote    ' + write_app(blob))
    report = u'\n'.join(L) + u'\n'
    if a.report:
